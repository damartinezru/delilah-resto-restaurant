...
const obtenerUsuario = async (entidadUsuario) => {
  ...
  return contextoUsuario.contextoBD
    .query(
      `SELECT * FROM usuario WHERE login = ? AND contrasena = ?`,
...
const agregarUsuario = async (entidadUsuario) => {
  ...
  return contextoUsuario.contextoBD
    .query(
      "INSERT INTO usuario(nombre,apellido,correo,login,contrasena,telefono,direccion,pais,ciudad,es_admin) VALUES (?,?,?,?,?,?,?,?,?,?)",
      {
        replacements: [
          usuario.nombre,
          usuario.apellido,
